...
export const Vortex = (props: VortexProps) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener("resize", () => {
      const canvas = canvasRef.current;
      if (canvas) {
        handleResize(canvas); // Fix: Removed `ctx` since it's not used
      }
    });
    return () => {
      window.removeEventListener("resize", () => {
        const canvas = canvasRef.current;
        if (canvas) {
          handleResize(canvas); // Fix: Ensure removal also works properly
        }
      });
    };
  }, []);
  ...
  return (
    <div className={cn("relative h-full w-full", props.containerClassName)}>
      <motion.div ref={containerRef} className="absolute h-full w-full inset-0 z-0 bg-transparent flex items-center justify-center">
